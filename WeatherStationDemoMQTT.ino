 ...
 * Display Settings
 **************************/
const int a = 12;  //For displaying segment "a"   D 06   
const int b = 13;  //For displaying segment "b"   D 07
const int c = 1;  //For displaying segment "c"   TX
const int d = 3;  //For displaying segment "d"   RX
const int e = 15;  //For displaying segment "e"   D 08
const int f = 4;  //For displaying segment "f"   D 02
const int g = 5;  //For displaying segment "g"   D 01   


/***************************
 * WIFI Settings
 **************************/
const char* WIFI_SSID = "Serafina";
const char* WIFI_PWD = "11235813";
...
const char* mqtt_server = "mqtt.fi.mdp.edu.ar";
...
String OPEN_WEATHER_MAP_LOCATION = "Mar del Plata, AR";
...
//void notificaciones(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y);
...
//********************* CHANGE PIN FUNCTION **********************
pinMode(c, FUNCTION_3); //GPIO 1 swap the pin to a GPIO.
pinMode(d, FUNCTION_3); //GPIO 3 swap the pin to a GPIO.
pinMode(e, FUNCTION_3); 
//****************************************************************
pinMode(c, OUTPUT);
pinMode(d, OUTPUT);
pinMode(a, OUTPUT);
...
//  Serial.println("iniciando");
  ...
  mqttClient.subscribe("inPrueba/");
  //mqttClient.subscribe("inDisp");
}
...
/*void notificaciones(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y) {
  display->setFont(ArialMT_Plain_10);
  display->setTextAlignment(TEXT_ALIGN_CENTER);
  display->drawString(64 + x, 1 + y, "Notificaciones: ");

  display->setFont(ArialMT_Plain_24);
  display->setTextAlignment(TEXT_ALIGN_LEFT);
  
  //String notif = String(getNotif);
 // display->drawString(60 + x, 5 + y, arr);
...
bgn:
  ...
  temp = chr[16] * 128 + chr[17] * 64 + chr[18] * 32 + chr[19] * 16 + chr[20] * 8 + chr[21] * 4 + chr[22] * 2 + chr[23];
    ...
    Serial.print("temp:");
    Serial.print(temp);
    //mqttClient.publish("outTemp", temp);
    mqttClient.publish("outTemp", String(temp).c_str(), true);
    Serial.print("    humi:");
    Serial.println(humi);
    //mqttClient.publish("outHumi", humi);
    mqttClient.publish("outHumi", String(humi).c_str(), true);

    if (temp > 25){
      mqttClient.publish("outAlerta", "ALERTA: temperatura superior a 25", true);
    }
    else
      if (temp < 15){
        mqttClient.publish("outAlerta", "ALERTA: temperatura inferior a 15", true);
      } else{
          mqttClient.publish("outAlerta", "sin alertas", true);
        }
...
void readLight(){
  ...
  mqttClient.publish("outLight", String(tempLight).c_str(), true);
...
void readAtmosphere(){
  ...
  mqttClient.publish("outPres", String(tempAtom).c_str(), true);
...
//  }
  ...
  //display.drawString(payload, 0, 5);

  // Switch on the LED if an 1 was received as first character
 // if ((char)payload[0] == '1') {
    
    //display.drawString(64, 10, "Prendido");
 ...
 // } else 
  //  if ((char)payload[0] == '0'){
    //display.drawString(64, 10, "Apagado");
